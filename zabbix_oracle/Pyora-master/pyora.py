...
class Checks(object):
    ...
    def tablespace(self, name):
            sql = '''select f.tablespace_name, round(((a.total - f.free) / a.total) * 100, 2) "% USED" from (select tablespace_name, round(sum(bytes / (1024 * 1024))) total from dba_data_files group by tablespace_name) a, (select tablespace_name, round(sum(bytes / (1024 * 1024))) free from dba_free_space group by tablespace_name) f WHERE a.tablespace_name = f.tablespace_name(+) AND a.tablespace_name='{0}' order by "% USED"'''.format(name)
            self.cur.execute(sql)
            res = self.cur.fetchall()
            for i in res:
                    print i[1]
